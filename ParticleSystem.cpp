...
namespace megadodo
{
  ...
  */
  ...
  {
    Vector4f position=(homAdd(list->position(index),geombias))*geomscale,
      position2=(homAdd(list->position(index),geombias2))*geomscale2;
    list->velocity(index)=makeVector3f(snoise4(position[0]+xbias[0],position[1]+xbias[1],position[2]+xbias[2],(owner->getTimeSinceStart()+xyztimebias[0])*timescale)*forcescale+
